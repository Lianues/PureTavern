/* eslint-disable */
// @ts-nocheck -- Browser-safe, line-for-line compatibility port of SillyTavern 1.18.0/src/prompt-converters.js.
import { sha512 } from 'js-sha512';

import { getUpstreamConfigValue } from './upstream-config';

const getConfigValue = (path, fallback) => getUpstreamConfigValue(path, fallback);
...
const randomBytesBase64 = (length) => {
...
export function convertMistralMessages(messages, names) {
    ...
    const sanitizeToolId = (id) => sha512(id).slice(0, 9);
...
export function embedOpenRouterMedia(
    messages: any[],
    { audio = true, video = true }: { audio?: boolean; video?: boolean } = { audio: true, video: true },
) {
    if (!Array.isArray(messages)) {
        return;
    }
