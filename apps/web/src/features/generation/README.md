...
Remote reference implementations live in `apps/remote-server`; native transports and their shell-only adapters live in `apps/mobile`, `apps/harmony` and `apps/desktop`. Direct mode can still fail because of CORS, TLS, Private Network Access or vendor policy; these failures remain reported as `cors-or-network`. Android, iOS, Harmony and desktop local modes bypass browser CORS through native HTTP APIs. Android and desktop explicitly permit user-configured HTTP LAN providers; iOS and Harmony accept HTTP URLs but retain the operating system's default cleartext policy. HTTPS is strongly preferred. Remote mode requires the proxy itself to be reachable and CORS-enabled. An HTTPS page may not call an HTTP LAN proxy because of Mixed Content/PNA, so production Web deployments need an HTTPS backend.
