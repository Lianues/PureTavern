...
- `/api/settings/get`、`save` 和四条 settings snapshot 路径已通过 Settings Port 接入 IndexedDB；原版 UI 继续发送完整 settings 文档并使用原版快照弹窗。
- 其他 `api-compat` 路径仍只提供让 UI 完成启动所需的空数据或安全默认响应，不代表对应业务模块已经迁移。
- 未覆盖端点返回 501，并记录到 `globalThis.__PURE_TAVERN__.diagnostics`。
- `globalThis.__PURE_TAVERN__.settingsStorage` 与 `settingsSnapshotStorage` 提供 IndexedDB/内存降级诊断。
